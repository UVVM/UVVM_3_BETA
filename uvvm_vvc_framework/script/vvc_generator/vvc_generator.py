...
def add_leaf_includes(file_handle, vvc_name, vvc_channel_idx, features):
    ...
    if features["transaction_pkg"]:
        file_handle.write("use work.vvc_transaction_pkg.all;\n")
        file_handle.write("use work.vvc_transaction_shared_variables_pkg.all;\n")
    file_handle.write("use work.vvc_cmd_pkg.all;\n")
    file_handle.write("use work.vvc_cmd_shared_variables_pkg.all;\n")
    ...
    file_handle.write("use work.vvc_methods_support_pkg.all;\n")
...
def add_architecture_declaration(file_handle, vvc_name, vvc_channel, features, num_of_queues):
    ...
    print_linefeed(file_handle)
    if vvc_channel.name != "NA":
        file_handle.write("  constant C_CHANNEL    : t_channel     := " + vvc_channel.name.upper() + ";\n")
    file_handle.write("  constant C_SCOPE      : string        := get_scope_for_log(C_VVC_NAME, GC_INSTANCE_IDX")
    if vvc_channel.name == "NA":
        file_handle.write(");\n")
    else:
        file_handle.write(", C_CHANNEL);\n")
    file_handle.write("  constant C_VVC_LABELS : t_vvc_labels  := assign_vvc_labels(C_VVC_NAME,")
    if vvc_channel.name == "NA":
    ...
    if vvc_channel.name == "NA":
        ...
        if features["transaction_info"]:
            file_handle.write("  -- Transaction info\n")
            file_handle.write("  alias vvc_transaction_info_trigger        : std_logic           is global_" + vvc_name.lower() + "_vvc_transaction_trigger(C_CHANNEL, GC_INSTANCE_IDX);\n")

    file_handle.write("  -- VVC Activity \n")
    file_handle.write("  signal entry_num_in_vvc_activity_register : integer;\n")
    print_linefeed(file_handle)

    file_handle.write("  impure function get_vvc_config(\n")
    file_handle.write("    constant void : t_void\n")
    file_handle.write("  ) return t_vvc_config is\n")
    file_handle.write("  begin\n")
    if vvc_channel.name == "NA":
        file_handle.write("    return shared_vvc_config.get(GC_INSTANCE_IDX);\n")
    else:
        file_handle.write("    return shared_vvc_config.get(GC_INSTANCE_IDX, C_CHANNEL);\n")
    file_handle.write("  end function get_vvc_config;\n")
    print_linefeed(file_handle)

    file_handle.write("  impure function get_vvc_status(\n")
    file_handle.write("    constant void : t_void\n")
    file_handle.write("  ) return t_vvc_status is\n")
    file_handle.write("  begin\n")
    if vvc_channel.name == "NA":
        file_handle.write("    return shared_vvc_status.get(GC_INSTANCE_IDX);\n")
    else:
        file_handle.write("    return shared_vvc_status.get(GC_INSTANCE_IDX, C_CHANNEL);\n")
    file_handle.write("  end function get_vvc_status;\n")
    print_linefeed(file_handle)

    file_handle.write("  impure function get_msg_id_panel(\n")
    file_handle.write("    constant void : t_void\n")
    file_handle.write("  ) return t_msg_id_panel is\n")
    file_handle.write("  begin\n")
    if vvc_channel.name == "NA":
        file_handle.write("    return shared_vvc_msg_id_panel.get(GC_INSTANCE_IDX);\n")
    else:
        file_handle.write("    return shared_vvc_msg_id_panel.get(GC_INSTANCE_IDX, C_CHANNEL);\n")
    file_handle.write("  end function get_msg_id_panel;\n")
    print_linefeed(file_handle)

    file_handle.write("  procedure set_vvc_status(\n")
    file_handle.write("    constant vvc_status : t_vvc_status\n")
    file_handle.write("  ) is\n")
    file_handle.write("  begin\n")
    if vvc_channel.name == "NA":
        file_handle.write("    shared_vvc_status.set(vvc_status, GC_INSTANCE_IDX);\n")
    else:
        file_handle.write("    shared_vvc_status.set(vvc_status, GC_INSTANCE_IDX, C_CHANNEL);\n")
    file_handle.write("  end procedure set_vvc_status;\n")
    print_linefeed(file_handle)

    file_handle.write("  procedure set_msg_id_panel(\n")
    file_handle.write("    constant msg_id_panel : t_msg_id_panel\n")
    file_handle.write("  ) is\n")
    file_handle.write("  begin\n")
    if vvc_channel.name == "NA":
        file_handle.write("    shared_vvc_msg_id_panel.set(msg_id_panel, GC_INSTANCE_IDX);\n")
    else:
        file_handle.write("    shared_vvc_msg_id_panel.set(msg_id_panel, GC_INSTANCE_IDX, C_CHANNEL);\n")
    file_handle.write("  end procedure set_msg_id_panel;\n")
    print_linefeed(file_handle)
...
def add_vvc_constructor(file_handle, vvc_name, vvc_channel):
    file_handle.write(division_line + "\n")
    file_handle.write("-- Constructor\n")
    file_handle.write("-- - Set up the defaults and show constructor if enabled\n")
    file_handle.write(division_line + "\n")
    if vvc_channel.name == "NA":
        file_handle.write("  vvc_constructor(C_SCOPE, GC_INSTANCE_IDX, shared_vvc_config, get_msg_id_panel(VOID), command_queue, result_queue, GC_" + vvc_name.upper() +
                          "_BFM_CONFIG,\n")
    else:
        file_handle.write("  vvc_constructor(C_SCOPE, GC_INSTANCE_IDX, C_CHANNEL, shared_vvc_config, get_msg_id_panel(VOID), command_queue, result_queue, GC_" + vvc_name.upper() +
                          "_BFM_CONFIG,\n")
...
def add_vvc_interpreter(file_handle, vvc_channel, features, num_of_queues):
    ...
    file_handle.write("    variable v_vvc_status    : t_vvc_status;\n")
    file_handle.write("  begin\n")
    print_linefeed(file_handle)
    file_handle.write("    -- 0. Initialize the process prior to first command\n")
    file_handle.write("    work.td_vvc_entity_support_pkg.initialize_interpreter(terminate_current_cmd);\n")
    file_handle.write("    -- initialise shared_vvc_last_received_cmd_idx for channel and instance\n")
    if vvc_channel.name == "NA":
        file_handle.write("    shared_vvc_last_received_cmd_idx.set(0, GC_INSTANCE_IDX);\n")
    else:
        file_handle.write("    shared_vvc_last_received_cmd_idx.set(0, GC_INSTANCE_IDX, C_CHANNEL);\n")
    print_linefeed(file_handle)
    ...
    if vvc_channel.name != "NA":
        file_handle.write("                                                                                         channel   => C_CHANNEL,\n")
    if num_of_queues == 1:
    ...
    print_linefeed(file_handle)
    file_handle.write("      v_msg_id_panel := get_msg_id_panel(VOID);\n")
    print_linefeed(file_handle)
    ...
    file_handle.write("      -------------------------------------------------------------------------\n")
    file_handle.write("      work.td_vvc_entity_support_pkg.await_cmd_from_sequencer(C_VVC_LABELS, C_SCOPE, v_msg_id_panel, THIS_VVCT, VVC_BROADCAST, global_vvc_busy," +
                      " global_vvc_ack, v_local_vvc_cmd);\n")
    file_handle.write("      -- Update shared_vvc_last_received_cmd_idx with received command index\n")
    if vvc_channel.name == "NA":
        file_handle.write("      shared_vvc_last_received_cmd_idx.set(v_local_vvc_cmd.cmd_idx, GC_INSTANCE_IDX);\n")
    else:
        file_handle.write("      shared_vvc_last_received_cmd_idx.set(v_local_vvc_cmd.cmd_idx, GC_INSTANCE_IDX, C_CHANNEL);\n")
    ...
    file_handle.write("      v_msg_id_panel := get_msg_id_panel(v_local_vvc_cmd, v_msg_id_panel);\n")
    print_linefeed(file_handle)
    ...
    file_handle.write("        v_vvc_status := get_vvc_status(VOID);\n")
    file_handle.write("        work.td_vvc_entity_support_pkg.put_command_on_queue(v_local_vvc_cmd, command_queue, v_vvc_status, queue_is_increasing);\n")
    file_handle.write("        set_vvc_status(v_vvc_status);\n")
    print_linefeed(file_handle)
    ...
    file_handle.write("            uvvm_util.methods_pkg.disable_log_msg(v_local_vvc_cmd.msg_id, v_msg_id_panel" +
    ...
    file_handle.write("            uvvm_util.methods_pkg.enable_log_msg(v_local_vvc_cmd.msg_id, v_msg_id_panel" +
                      ", to_string(v_local_vvc_cmd.msg) & format_command_idx(v_local_vvc_cmd), C_SCOPE, v_local_vvc_cmd.quietness);\n")
    print_linefeed(file_handle)
    file_handle.write("          when FLUSH_COMMAND_QUEUE =>\n")
    file_handle.write("            v_vvc_status := get_vvc_status(VOID);\n")
    file_handle.write("            work.td_vvc_entity_support_pkg.interpreter_flush_command_queue(v_local_vvc_cmd, command_queue, v_msg_id_panel, v_vvc_status," +
                      " C_VVC_LABELS, C_SCOPE);\n")
    file_handle.write("            set_vvc_status(v_vvc_status);\n")
    print_linefeed(file_handle)
    file_handle.write("          when TERMINATE_CURRENT_COMMAND =>\n")
    file_handle.write("            work.td_vvc_entity_support_pkg.interpreter_terminate_current_command(v_local_vvc_cmd, v_msg_id_panel, C_VVC_LABELS, C_SCOPE," +
                      " terminate_current_cmd, executor_is_busy);\n")
    print_linefeed(file_handle)
    file_handle.write("          when FETCH_RESULT =>\n")
    file_handle.write("            work.td_vvc_entity_support_pkg.interpreter_fetch_result(result_queue, entry_num_in_vvc_activity_register, v_local_vvc_cmd, v_msg_id_panel, C_VVC_LABELS, C_SCOPE," +
                      " shared_vvc_response);\n")
    print_linefeed(file_handle)
    ...
    print_linefeed(file_handle)
    file_handle.write("        set_msg_id_panel(v_msg_id_panel);\n")
    print_linefeed(file_handle)
...
def add_vvc_executor(file_handle, vvc_name, vvc_channel, vvc_channel_idx, features):
    ...
    file_handle.write("    variable v_msg_id_panel                           : t_msg_id_panel;\n")
    file_handle.write("    variable v_vvc_config                             : t_vvc_config;\n")
    ...
    if vvc_channel.name == "NA":
        file_handle.write("      update_vvc_activity_register(global_trigger_vvc_activity_register, shared_vvc_status, GC_INSTANCE_IDX, NA, INACTIVE, entry_num_in_vvc_activity_register," +
                          "  C_EXECUTOR_ID, last_cmd_idx_executed, command_queue.is_empty(VOID), C_SCOPE);\n")
    else:
        file_handle.write("      update_vvc_activity_register(global_trigger_vvc_activity_register, shared_vvc_status, GC_INSTANCE_IDX, C_CHANNEL, INACTIVE, entry_num_in_vvc_activity_register," +
                          "  C_EXECUTOR_ID, last_cmd_idx_executed, command_queue.is_empty(VOID), C_SCOPE);\n")
    print_linefeed(file_handle)

    file_handle.write("      -- 1. Set defaults, fetch command and log\n")
    file_handle.write("      -------------------------------------------------------------------------\n")
    file_handle.write("      work.td_vvc_entity_support_pkg.fetch_command_and_prepare_executor(v_cmd, command_queue, shared_vvc_status, queue_is_increasing," +
                      " executor_is_busy, C_VVC_LABELS, C_SCOPE);\n")
    print_linefeed(file_handle)

    file_handle.write("      -- update vvc activity\n")
    if vvc_channel.name == "NA":
        file_handle.write("      update_vvc_activity_register(global_trigger_vvc_activity_register, shared_vvc_status, GC_INSTANCE_IDX, NA, ACTIVE, entry_num_in_vvc_activity_register," +
                          " C_EXECUTOR_ID, last_cmd_idx_executed, command_queue.is_empty(VOID), C_SCOPE);\n")
    else:
        file_handle.write("      update_vvc_activity_register(global_trigger_vvc_activity_register, shared_vvc_status, GC_INSTANCE_IDX, C_CHANNEL, ACTIVE, entry_num_in_vvc_activity_register," +
                          " C_EXECUTOR_ID, last_cmd_idx_executed, command_queue.is_empty(VOID), C_SCOPE);\n")
    print_linefeed(file_handle)

    file_handle.write("      v_vvc_config := get_vvc_config(VOID);\n")
    print_linefeed(file_handle)

    file_handle.write("      -- Select between a provided msg_id_panel via the vvc_cmd_record from a VVC with a higher hierarchy or the\n")
    file_handle.write("      -- msg_id_panel in this VVC's config. This is to correctly handle the logging when using Hierarchical-VVCs.\n")
    file_handle.write("      v_msg_id_panel := get_msg_id_panel(v_cmd, get_msg_id_panel(VOID));\n")
    print_linefeed(file_handle)
    ...
    file_handle.write("      work.td_vvc_entity_support_pkg.insert_inter_bfm_delay_if_requested(vvc_config                         => v_vvc_config,\n")
    ...
    if features["transaction_info"]:
        ...
        if vvc_channel.name == "NA":
            file_handle.write("        --     set_global_vvc_transaction_info(vvc_transaction_info_trigger, shared_" + vvc_name.lower() + "_vvc_transaction_info, GC_INSTANCE_IDX, NA, v_cmd, v_vvc_config, IN_PROGRESS, C_SCOPE);\n")
        else:
            file_handle.write("        --     set_global_vvc_transaction_info(vvc_transaction_info_trigger, shared_" + vvc_name.lower() + "_vvc_transaction_info, GC_INSTANCE_IDX, C_CHANNEL, v_cmd, v_vvc_config, IN_PROGRESS, C_SCOPE);\n")
        print_linefeed(file_handle)
    ...
    file_handle.write("        --               scope         => C_SCOPE,\n")
    file_handle.write("        --               msg_id_panel  => v_msg_id_panel,\n")
    file_handle.write("        --               config        => v_vvc_config.bfm_config);\n")
    print_linefeed(file_handle)

    if features["transaction_info"]:
        file_handle.write("        --     -- Update vvc transaction info\n")
        if vvc_channel.name == "NA":
            file_handle.write("        --     set_global_vvc_transaction_info(vvc_transaction_info_trigger, shared_" + vvc_name.lower() + "_vvc_transaction_info, GC_INSTANCE_IDX, NA, v_cmd, v_vvc_config, COMPLETED, C_SCOPE);\n")
        else:
            file_handle.write("        --     set_global_vvc_transaction_info(vvc_transaction_info_trigger, shared_" + vvc_name.lower() + "_vvc_transaction_info, GC_INSTANCE_IDX, C_CHANNEL, v_cmd, v_vvc_config, COMPLETED, C_SCOPE);\n")
        print_linefeed(file_handle)
    ...
    if number_of_executors > 1:
        ...
        if features["transaction_info"]:
            ...
            file_handle.write("        --       -- Update vvc transaction info\n")
            if vvc_channel.name == "NA":
                file_handle.write("        --       set_global_vvc_transaction_info(vvc_transaction_info_trigger, shared_" + vvc_name.lower() + "_vvc_transaction_info, GC_INSTANCE_IDX, NA, v_cmd, v_result, COMPLETED, C_SCOPE);\n")
            else:
                file_handle.write("        --       set_global_vvc_transaction_info(vvc_transaction_info_trigger, shared_" + vvc_name.lower() + "_vvc_transaction_info, GC_INSTANCE_IDX, C_CHANNEL, v_cmd, v_result, COMPLETED, C_SCOPE);\n")
    ...
    file_handle.write("            -- check_value(v_vvc_config.bfm_config.clock_period > -1 ns, TB_ERROR, \"Check that clock_period is configured when using insert_delay().\",\n")
    file_handle.write("            --             C_SCOPE, ID_NEVER, v_msg_id_panel);\n")
    file_handle.write("            -- wait until terminate_current_cmd.is_active = '1' for v_cmd.gen_integer_array(0) * v_vvc_config.bfm_config.clock_period;\n")
    file_handle.write("          end if;\n")
    print_linefeed(file_handle)
    file_handle.write("        when others =>\n")
    ...
    file_handle.write("        if ((v_vvc_config.inter_bfm_delay.delay_type = TIME_START2START) and \n")
    file_handle.write("           ((now - v_timestamp_start_of_current_bfm_access) > v_vvc_config.inter_bfm_delay.delay_in_time)) then\n")
    file_handle.write("          alert(v_vvc_config.inter_bfm_delay.inter_bfm_delay_violation_severity, \"BFM access exceeded specified start-to-start inter-bfm delay, \" & \n")
    file_handle.write("                to_string(v_vvc_config.inter_bfm_delay.delay_in_time) & \".\", C_SCOPE);\n")
    file_handle.write("        end if;\n")
    file_handle.write("      end if;\n")
    print_linefeed(file_handle)
    ...
    if features["transaction_info"]:
        ...
        if vvc_channel.name == "NA":
            file_handle.write("        --    reset_vvc_transaction_info(shared_" + vvc_name.lower() + "_vvc_transaction_info, GC_INSTANCE_IDX, NA, v_cmd);\n")
        else:
            file_handle.write("        --    reset_vvc_transaction_info(shared_" + vvc_name.lower() + "_vvc_transaction_info, GC_INSTANCE_IDX, C_CHANNEL, v_cmd);\n")
        print_linefeed(file_handle)
    file_handle.write("    end loop;\n")
    file_handle.write("  end process;\n")
    file_handle.write(division_line + "\n")
...
def add_vvc_pipeline_step(file_handle, vvc_name, vvc_channel, queue_name, executor_idx, features):
    file_handle.write(division_line + "\n")
    ...
    file_handle.write("      -------------------------------------------------------------------------\n")
    file_handle.write("      work.td_vvc_entity_support_pkg.fetch_command_and_prepare_executor(v_cmd, " + queue_name + "_queue" +
                      ", shared_vvc_status, " + queue_name + "_queue_is_increasing, " + queue_name + "_is_busy, C_VVC_LABELS, C_SCOPE);\n")
    print_linefeed(file_handle)
    file_handle.write("      -- update vvc activity\n")
    if vvc_channel.name == "NA":
        file_handle.write("      update_vvc_activity_register(global_trigger_vvc_activity_register, shared_vvc_status, GC_INSTANCE_IDX, NA, ACTIVE, entry_num_in_vvc_activity_register," +
...
def add_unwanted_activity_detection(file_handle, vvc_name, vvc_channel):
    ...
    file_handle.write("        -- v_vvc_config := get_vvc_config(VOID);\n")
    if vvc_channel != "NA":
        file_handle.write("        -- check_value(not t_" + vvc_name.lower() + "_" + vvc_channel.lower() + "_if.last'event, v_vvc_config.unwanted_activity_severity, \"Unwanted activity detected on last\", C_SCOPE, ID_NEVER, get_msg_id_panel(VOID));\n")
        file_handle.write("        -- check_value(not t_" + vvc_name.lower() + "_" + vvc_channel.lower() + "_if.data'event, v_vvc_config.unwanted_activity_severity, \"Unwanted activity detected on data\", C_SCOPE, ID_NEVER, get_msg_id_panel(VOID));\n")
    else:
        file_handle.write("        -- check_value(not t_" + vvc_name.lower() + "_if.last'event, v_vvc_config.unwanted_activity_severity, \"Unwanted activity detected on last\", C_SCOPE, ID_NEVER, get_msg_id_panel(VOID));\n")
        file_handle.write("        -- check_value(not t_" + vvc_name.lower() + "_if.data'event, v_vvc_config.unwanted_activity_severity, \"Unwanted activity detected on data\", C_SCOPE, ID_NEVER, get_msg_id_panel(VOID));\n")
    print_linefeed(file_handle)
    ...
    if vvc_channel != "NA":
        ...
        file_handle.write("        --   check_value(not t_" + vvc_name.lower() + "_" + vvc_channel.lower() + "_if.valid'event, v_vvc_config.unwanted_activity_severity, \"Unwanted activity detected on valid\", C_SCOPE, ID_NEVER, get_msg_id_panel(VOID));\n")
        file_handle.write("        --   check_value(not t_" + vvc_name.lower() + "_" + vvc_channel.lower() + "_if.data'event, v_vvc_config.unwanted_activity_severity, \"Unwanted activity detected on data\", C_SCOPE, ID_NEVER, get_msg_id_panel(VOID));\n")
    else:
        ...
        file_handle.write("        --   check_value(not t_" + vvc_name.lower() + "_if.valid'event, v_vvc_config.unwanted_activity_severity, \"Unwanted activity detected on valid\", C_SCOPE, ID_NEVER, get_msg_id_panel(VOID));\n")
        file_handle.write("        --   check_value(not t_" + vvc_name.lower() + "_if.data'event, v_vvc_config.unwanted_activity_severity, \"Unwanted activity detected on data\", C_SCOPE, ID_NEVER, get_msg_id_panel(VOID));\n")
    file_handle.write("        -- end if;\n")
    file_handle.write("      end if;\n")
    file_handle.write("    end loop;\n")
...
def add_vvc_cmd_pkg_includes(file_handle, features):
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("--  VVC command package\n")
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("library ieee;\n")
    file_handle.write("use ieee.std_logic_1164.all;\n")
    file_handle.write("use ieee.numeric_std.all;\n")
...
def add_vvc_cmd_pkg_header(file_handle, features):
    ...
    print_linefeed(file_handle)
    if features["transaction_pkg"]:
        file_handle.write("  alias t_operation is work.vvc_transaction_pkg.t_operation;\n")
        print_linefeed(file_handle)
    ...
    file_handle.write("  end record;\n")
    print_linefeed(file_handle)
    file_handle.write("  constant C_VVC_RESPONSE_DEFAULT : t_vvc_response := (\n")
    file_handle.write("    fetch_is_accepted  => false,\n")
    file_handle.write("    transaction_result => ACK,\n")
    file_handle.write("    result             => (others => '0')\n")
    file_handle.write("  );\n")
    print_linefeed(file_handle)
    file_handle.write("  " + division_line + "\n")
...
def add_vvc_cmd_pkg_body(file_handle, features):
    ...
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("--  Generic package instantiations\n")
    file_handle.write(division_line_pkg + "\n")
    file_handle.write(division_line_sub + "\n")
    file_handle.write("-- Protected type: t_vvc_cmd_record\n")
    file_handle.write(division_line_sub + "\n")
    file_handle.write("library uvvm_util;\n")
    file_handle.write("use work.vvc_cmd_pkg.all;\n")
    if features["transaction_info"]:
        file_handle.write("use work.vvc_transaction_pkg.all;\n")
    print_linefeed(file_handle)
    file_handle.write("package protected_vvc_cmd_pkg is new uvvm_util.protected_generic_types_pkg\n")
    file_handle.write("  generic map(\n")
    file_handle.write("    t_generic_element  => t_vvc_cmd_record,\n")
    file_handle.write("    c_generic_default  => C_VVC_CMD_DEFAULT,\n")
    file_handle.write("    c_max_instance_num => C_VVC_MAX_INSTANCE_NUM\n")
    file_handle.write("  );\n")
    print_linefeed(file_handle)
    file_handle.write(division_line_sub + "\n")
    file_handle.write("-- Protected type: t_vvc_response\n")
    file_handle.write(division_line_sub + "\n")
    file_handle.write("library uvvm_util;\n")
    file_handle.write("use work.vvc_cmd_pkg.all;\n")
    if features["transaction_info"]:
        file_handle.write("use work.vvc_transaction_pkg.all;\n")
    print_linefeed(file_handle)
    file_handle.write("package protected_vvc_response_pkg is new uvvm_util.protected_generic_types_pkg\n")
    file_handle.write("  generic map(\n")
    file_handle.write("    t_generic_element  => t_vvc_response,\n")
    file_handle.write("    c_generic_default  => C_VVC_RESPONSE_DEFAULT,\n")
    file_handle.write("    c_max_instance_num => C_VVC_MAX_INSTANCE_NUM\n")
    file_handle.write("  );\n")
    print_linefeed(file_handle)
    file_handle.write(division_line_sub + "\n")
    file_handle.write("-- Protected type: vvc_last_received_cmd_idx\n")
    file_handle.write(division_line_sub + "\n")
    file_handle.write("library uvvm_util;\n")
    file_handle.write("use work.vvc_cmd_pkg.all;\n")
    if features["transaction_info"]:
        file_handle.write("use work.vvc_transaction_pkg.all;\n")
    print_linefeed(file_handle)
    file_handle.write("package protected_vvc_last_received_cmd_idx_pkg is new uvvm_util.protected_generic_types_pkg\n")
    file_handle.write("  generic map(\n")
    file_handle.write("    t_generic_element  => integer,\n")
    file_handle.write("    c_generic_default  => -1,\n")
    file_handle.write("    c_max_instance_num => C_VVC_MAX_INSTANCE_NUM\n")
    file_handle.write("  );\n")
    print_linefeed(file_handle)
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("--  Shared variables package\n")
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("use work.protected_vvc_cmd_pkg.all;\n")
    file_handle.write("use work.protected_vvc_response_pkg.all;\n")
    file_handle.write("use work.protected_vvc_last_received_cmd_idx_pkg.all;\n")
    print_linefeed(file_handle)
    file_handle.write("package vvc_cmd_shared_variables_pkg is\n")
    file_handle.write("  shared variable shared_vvc_cmd                   : work.protected_vvc_cmd_pkg.t_generic_array;\n")
    file_handle.write("  shared variable shared_vvc_response              : work.protected_vvc_response_pkg.t_generic_array;\n")
    file_handle.write("  shared variable shared_vvc_last_received_cmd_idx : work.protected_vvc_last_received_cmd_idx_pkg.t_generic_array;\n")
    file_handle.write("end package vvc_cmd_shared_variables_pkg;\n")

def add_methods_pkg_includes(file_handle, vvc_name, features):
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("--  Support package\n")
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("library uvvm_util;\n")
    file_handle.write("use uvvm_util.adaptations_pkg.all;\n")
    file_handle.write("use uvvm_util.types_pkg.all;\n")
    file_handle.write("use work." + vvc_name.lower() + "_bfm_pkg.all;\n")
    file_handle.write("use work.td_target_support_pkg.all;\n")
    print_linefeed(file_handle)
    file_handle.write("package vvc_methods_support_pkg is\n")
    print_linefeed(file_handle)
    file_handle.write("  " + division_line + "\n")
    ...
    file_handle.write("    inter_bfm_delay 			     : t_inter_bfm_delay; -- Minimum delay between BFM accesses from the VVC. If parameter delay_type is set to NO_DELAY, BFM accesses will be back to back, i.e. no delay.\n")
    ...
    file_handle.write("  );\n")
    print_linefeed(file_handle)
    file_handle.write("end package vvc_methods_support_pkg;\n")
    print_linefeed(file_handle)
    ...
    file_handle.write(division_line_pkg + "\n")
    file_handle.write(division_line_sub + "\n")
    file_handle.write("-- Protected type: t_vvc_status\n")
    file_handle.write(division_line_sub + "\n")
    file_handle.write("library uvvm_util;\n")
    file_handle.write("use work.vvc_methods_support_pkg.all;\n")
    if features["transaction_info"]:
        ...
        file_handle.write("use work.vvc_cmd_pkg.all;\n")
    print_linefeed(file_handle)
    file_handle.write("package protected_vvc_status_pkg is new uvvm_util.protected_generic_types_pkg\n")
    file_handle.write("  generic map(\n")
    file_handle.write("    t_generic_element  => t_vvc_status,\n")
    file_handle.write("    c_generic_default  => C_VVC_STATUS_DEFAULT,\n")
    file_handle.write("    c_max_instance_num => C_VVC_MAX_INSTANCE_NUM\n")
    file_handle.write("  );\n")
    print_linefeed(file_handle)
    file_handle.write(division_line_sub + "\n")
    file_handle.write("-- Protected type: t_vvc_config\n")
    file_handle.write(division_line_sub + "\n")
    file_handle.write("library uvvm_util;\n")
    file_handle.write("use work.vvc_methods_support_pkg.all;\n")
    if features["transaction_info"]:
        ...
        file_handle.write("use work.vvc_cmd_pkg.all;\n")
    print_linefeed(file_handle)
    file_handle.write("package protected_vvc_config_pkg is new uvvm_util.protected_generic_types_pkg\n")
    file_handle.write("  generic map(\n")
    file_handle.write("    t_generic_element  => t_vvc_config,\n")
    file_handle.write("    c_generic_default  => C_" + vvc_name.upper() + "_VVC_CONFIG_DEFAULT,\n")
    file_handle.write("    c_max_instance_num => C_VVC_MAX_INSTANCE_NUM\n")
    file_handle.write("  );\n")
    print_linefeed(file_handle)
    file_handle.write(division_line_sub + "\n")
    file_handle.write("-- Protected type: t_msg_id_panel\n")
    file_handle.write(division_line_sub + "\n")
    file_handle.write("library uvvm_util;\n")
    file_handle.write("use uvvm_util.types_pkg.all;\n")
    file_handle.write("use uvvm_util.adaptations_pkg.all;\n")
    if features["transaction_info"]:
        ...
        file_handle.write("use work.vvc_cmd_pkg.all;\n")
    print_linefeed(file_handle)
    file_handle.write("package protected_msg_id_panel_pkg is new uvvm_util.protected_generic_types_pkg\n")
    file_handle.write("  generic map(\n")
    file_handle.write("    t_generic_element  => t_msg_id_panel,\n")
    file_handle.write("    c_generic_default  => C_VVC_MSG_ID_PANEL_DEFAULT,\n")
    file_handle.write("    c_max_instance_num => C_VVC_MAX_INSTANCE_NUM\n")
    file_handle.write("  );\n")
    print_linefeed(file_handle)

    file_handle.write(division_line_pkg + "\n")
    file_handle.write("--  VVC methods package\n")
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("library ieee;\n")
    file_handle.write("use ieee.std_logic_1164.all;\n")
    file_handle.write("use ieee.numeric_std.all;\n")
    ...
    if features["transaction_pkg"]:
        file_handle.write("use work.vvc_transaction_pkg.all;\n")
        file_handle.write("use work.protected_transaction_group_pkg.all;\n")
    file_handle.write("use work.vvc_cmd_pkg.all;\n")
    file_handle.write("use work.vvc_cmd_shared_variables_pkg.all;\n")
    file_handle.write("use work.td_target_support_pkg.all;\n")
    file_handle.write("use work.vvc_methods_support_pkg.all;\n")
    file_handle.write("use work.protected_vvc_status_pkg.all;\n")
    file_handle.write("use work.protected_vvc_config_pkg.all;\n")
    file_handle.write("use work.protected_msg_id_panel_pkg.all;\n")
    if features["scoreboard"]:
...
def add_methods_pkg_header(file_handle, vvc_name, vvc_channels, features):
    ...
    print_linefeed(file_handle)
    file_handle.write("  shared variable shared_" + vvc_name.lower() + "_vvc_status       : work.protected_vvc_status_pkg.t_generic_array;\n")
    file_handle.write("  shared variable shared_" + vvc_name.lower() + "_vvc_config       : work.protected_vvc_config_pkg.t_generic_array;\n")
    file_handle.write("  shared variable shared_" + vvc_name.lower() + "_vvc_msg_id_panel : work.protected_msg_id_panel_pkg.t_generic_array;\n")
    if features["scoreboard"]:
        file_handle.write("  shared variable " + vvc_name.upper() + "_VVC_SB                  : t_generic_sb;\n")
    print_linefeed(file_handle)
    file_handle.write("  alias shared_vvc_status       is shared_" + vvc_name.lower() + "_vvc_status;       -- This alias is for internal use in the VVC\n")
    file_handle.write("  alias shared_vvc_config       is shared_" + vvc_name.lower() + "_vvc_config;       -- This alias is for internal use in the VVC\n")
    file_handle.write("  alias shared_vvc_msg_id_panel is shared_" + vvc_name.lower() + "_vvc_msg_id_panel; -- This alias is for internal use in the VVC\n")
    print_linefeed(file_handle)
    file_handle.write("  " + division_line + "\n")
    ...
    if features["transaction_info"]:
        ...
        file_handle.write("    variable vvc_transaction_info_group   : inout work.protected_transaction_group_pkg.t_generic_array;\n")
        file_handle.write("    constant instance_idx                 : in natural;\n")
        file_handle.write("    constant channel                      : in t_channel;\n")
...
def add_methods_pkg_body(file_handle, vvc_name, features):
    ...
    file_handle.write("  --   constant proc_call : string := proc_name & \"(\" & to_string(VVCT, vvc_instance_idx)  -- First part common for all\n")
    ...
    file_handle.write("  --   variable v_normalised_addr    : unsigned(C_VVC_CMD_ADDR_MAX_LENGTH-1 downto 0) := \n"
                      "  --            normalize_and_check(addr, shared_vvc_cmd.addr, ALLOW_WIDER_NARROWER, \"addr\", \"shared_vvc_cmd.addr\", proc_call" +
    ...
    file_handle.write("  --   variable v_normalised_data    : std_logic_vector(C_VVC_CMD_DATA_MAX_LENGTH-1 downto 0) := \n"
                      "  --            normalize_and_check(data, shared_vvc_cmd.data, ALLOW_WIDER_NARROWER, \"data\", \"shared_vvc_cmd.data\", proc_call" +
    ...
    file_handle.write("  --   set_general_target_and_command_fields(VVCT, vvc_instance_idx, proc_call, msg, QUEUED, WRITE);\n")
    ...
    file_handle.write("  --   constant proc_call : string := proc_name & \"(\" & to_string(VVCT, vvc_instance_idx, channel) & \")\";\n")
    file_handle.write("  --   variable v_msg_id_panel : t_msg_id_panel := shared_msg_id_panel;\n")
    file_handle.write("  -- begin\n")
    file_handle.write("  -- -- Create command by setting common global 'VVCT' signal record and dedicated VVC 'shared_vvc_cmd' record\n")
    file_handle.write("  -- -- locking semaphore in set_general_target_and_command_fields to gain exclusive right to VVCT and shared_vvc_cmd\n")
    file_handle.write("  -- -- semaphore gets unlocked in await_cmd_from_sequencer of the targeted VVC\n")
    file_handle.write("  --   set_general_target_and_command_fields(VVCT, vvc_instance_idx, channel, proc_call, msg, QUEUED, RECEIVE);\n")
    ...
    if features["transaction_info"]:
        ...
        file_handle.write("    variable v_transaction_info_group : t_transaction_group := vvc_transaction_info_group.get(instance_idx, channel);\n")
        file_handle.write("  begin\n")
        ...
        file_handle.write("  --      v_transaction_info_group.bt.operation          := vvc_cmd.operation;\n")
        file_handle.write("  --      v_transaction_info_group.bt.address            := vvc_cmd.addr;\n")
        file_handle.write("  --      v_transaction_info_group.bt.data               := vvc_cmd.data;\n")
        file_handle.write("  --      v_transaction_info_group.bt.vvc_meta.msg       := vvc_cmd.msg;\n")
        file_handle.write("  --      v_transaction_info_group.bt.vvc_meta.cmd_idx   := vvc_cmd.cmd_idx;\n")
        file_handle.write("  --      v_transaction_info_group.bt.transaction_status := transaction_status;\n")
        file_handle.write("  --      vvc_transaction_info_group.set(v_transaction_info_group, instance_idx, channel);\n")
        ...
        file_handle.write("  --  case vvc_cmd.operation is\n")
        file_handle.write("  --    when READ =>\n")
        file_handle.write("  --      v_transaction_info_group.bt.data               := vvc_result;\n")
        file_handle.write("  --      v_transaction_info_group.bt.transaction_status := transaction_status;\n")
        file_handle.write("  --      vvc_transaction_info_group.set(v_transaction_info_group, instance_idx, channel);\n")
        file_handle.write("  --      gen_pulse(vvc_transaction_info_trigger, 0 ns, \"pulsing global vvc transaction info trigger\", scope, ID_NEVER);\n")
        print_linefeed(file_handle)
        ...
        file_handle.write("    constant instance_idx               : in natural;\n")
        file_handle.write("    constant channel                    : in t_channel;\n")
        file_handle.write("    constant vvc_cmd                    : in t_vvc_cmd_record) is\n")
        file_handle.write("    variable v_transaction_info_group : t_transaction_group := vvc_transaction_info_group.get(instance_idx, channel);\n")
        file_handle.write("  begin\n")
        file_handle.write("  -- <USER_INPUT> Please insert the VVC operations here.\n")
        file_handle.write("  --  case vvc_cmd.operation is\n")
        file_handle.write("  --    when WRITE | READ =>\n")
        file_handle.write("  --      v_transaction_info_group.bt := C_BASE_TRANSACTION_SET_DEFAULT;\n")
        print_linefeed(file_handle)
        file_handle.write("  --    when others =>\n")
...
def generate_vvc_cmd_pkg_file(features):
    ...
    add_vvc_cmd_pkg_body(f, features)
    f.close()
...
def add_transaction_pkg(file_handle, vvc_name, vvc_channels, features):
    ...
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("--  VVC transaction package\n")
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("library ieee;\n")
    file_handle.write("use ieee.std_logic_1164.all;\n")
    file_handle.write("use ieee.numeric_std.all;\n")
    ...
    file_handle.write("context uvvm_util.uvvm_util_context;\n")
    print_linefeed(file_handle)
    file_handle.write("package vvc_transaction_pkg is\n")
    print_linefeed(file_handle)
    file_handle.write("  " + division_line + "\n")
    file_handle.write("  -- t_operation\n")
    file_handle.write("  -- - VVC and BFM operations\n")
    ...
    print_linefeed(file_handle)
    file_handle.write("end package vvc_transaction_pkg;\n")
    ...
    if features["transaction_info"]:
        ...
        file_handle.write(division_line_pkg + "\n")
        file_handle.write(division_line_sub + "\n")
        file_handle.write("-- Protected type: t_transaction_group\n")
        file_handle.write(division_line_sub + "\n")
        file_handle.write("library uvvm_util;\n")
        file_handle.write("use work.vvc_transaction_pkg.all;\n")
        print_linefeed(file_handle)
        file_handle.write("package protected_transaction_group_pkg is new uvvm_util.protected_generic_types_pkg\n")
        file_handle.write("  generic map(\n")
        file_handle.write("    t_generic_element  => t_transaction_group,\n")
        file_handle.write("    c_generic_default  => C_TRANSACTION_GROUP_DEFAULT,\n")
        file_handle.write("    c_max_instance_num => C_VVC_MAX_INSTANCE_NUM\n")
        file_handle.write("  );\n")
        print_linefeed(file_handle)
        file_handle.write(division_line_pkg + "\n")
        ...
        file_handle.write("use work.protected_transaction_group_pkg.all;\n")
        print_linefeed(file_handle)
        file_handle.write("package vvc_transaction_shared_variables_pkg is\n")
        file_handle.write("  shared variable shared_" + vvc_name.lower() + "_vvc_transaction_info : work.protected_transaction_group_pkg.t_generic_array;\n")
        file_handle.write("end package vvc_transaction_shared_variables_pkg;\n")


def generate_transaction_pkg_file(vvc_name, vvc_channels, features):
    if features["transaction_pkg"]:
        f = open("output/vvc_transaction_pkg.vhd", 'w')
...
def add_vvc_sb_pkg(file_handle, vvc_name, vvc_channels, features):
    ...
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("--  VVC scoreboard package\n")
    file_handle.write(division_line_pkg + "\n")
    file_handle.write("library ieee;\n")
    file_handle.write("use ieee.std_logic_1164.all;\n")
    file_handle.write("use ieee.numeric_std.all;\n")
...
def add_vvc_context(file_handle, vvc_name, features):
    ...
    if features["transaction_pkg"]:
        file_handle.write("  use bitvis_vip_" + vvc_name.lower() + ".vvc_transaction_pkg.all;\n")
        file_handle.write("  use bitvis_vip_" + vvc_name.lower() + ".vvc_transaction_shared_variables_pkg.all;\n")
    file_handle.write("  use bitvis_vip_" + vvc_name.lower() + ".vvc_methods_pkg.all;\n")
    file_handle.write("  use bitvis_vip_" + vvc_name.lower() + ".vvc_methods_support_pkg.all;\n")
    if features["scoreboard"]:
        file_handle.write("  use bitvis_vip_" + vvc_name.lower() + ".vvc_sb_support_pkg.all;\n")
...
def generate_vvc_file(vvc_name, vvc_channels, features):
    ...
    for channel_idx, channel in enumerate(vvc_channels):
        ...
        add_vvc_constructor(f, vvc_name, channel)
        ...
        if (num_of_queues > 1):
            for i in range(1, num_of_queues):
                add_vvc_pipeline_step(f, vvc_name, channel, channel.executor_names[i], i, features)
