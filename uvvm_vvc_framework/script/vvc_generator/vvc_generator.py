...
def add_vvc_entity(file_handle, vvc_name, vvc_channel):
    ...
    file_handle.write("    GC_" + vvc_name.upper() + "_CONFIG" + fill_with_n_spaces(vvc_name.__len__(), 22) + 
...
def add_architecture_declaration(file_handle, vvc_name, vvc_channel, features, num_of_queues):
    ...
    file_handle.write("  shared variable command_queue : work.td_cmd_queue_pkg.t_prot_generic_queue;\n")

    if number_of_executors > 1:
        for i in range(1, number_of_executors):
            #file_handle.write("  shared variable " + vvc_channel.executor_names[i] + "_queue : work.td_cmd_queue_pkg.t_generic_queue;\n")
            file_handle.write("  shared variable {}_queue : work.td_cmd_queue_pkg.t_prot_generic_queue;\n".format(vvc_channel.executor_names[i]))

    # file_handle.write("  shared variable result_queue  : work.td_result_queue_pkg.t_generic_queue;\n")
    file_handle.write("  shared variable result_queue  : work.td_result_queue_pkg.t_prot_generic_queue;\n")
...
def add_vvc_constructor(file_handle, vvc_name, vvc_channel):
    ...
    file_handle.write(division_line + "\n")
    if vvc_channel.name == "NA":    
        file_handle.write("  vvc_constructor(C_SCOPE, GC_INSTANCE_IDX, shared_vvc_config, shared_vvc_msg_id_panel.get(GC_INSTANCE_IDX), command_queue, result_queue, GC_{}_CONFIG,\n".format(vvc_name.upper()))
    else:
        file_handle.write("  vvc_constructor(C_SCOPE, GC_INSTANCE_IDX, shared_vvc_config, shared_vvc_msg_id_panel.get(GC_INSTANCE_IDX, GC_CHANNEL), command_queue, result_queue, GC_{}_CONFIG,\n".format(vvc_name.upper()))
...
def add_vvc_interpreter(file_handle, vvc_channel, features, num_of_queues):
    ...
    if vvc_channel.name == "NA":
        ...
        file_handle.write("    shared_vvc_last_received_cmd_idx.set(0, GC_INSTANCE_IDX, GC_CHANNEL);\n")
    print_linefeed(file_handle)
    ...
    file_handle.write("      -------------------------------------------------------------------------\n")
    file_handle.write("      work.td_vvc_entity_support_pkg.await_cmd_from_sequencer(C_VVC_LABELS, vvc_config, THIS_VVCT, VVC_BROADCAST, global_vvc_busy, global_vvc_ack, v_local_vvc_cmd);\n")
    ...
    file_handle.write("            uvvm_util.methods_pkg.disable_log_msg(v_local_vvc_cmd.msg_id, vvc_config.msg_id_panel, to_string(v_local_vvc_cmd.msg) & format_command_idx(v_local_vvc_cmd), C_SCOPE, v_local_vvc_cmd.quietness);\n")
    print_linefeed(file_handle)
    file_handle.write("          when ENABLE_LOG_MSG =>\n")
    file_handle.write("            uvvm_util.methods_pkg.enable_log_msg(v_local_vvc_cmd.msg_id, vvc_config.msg_id_panel, to_string(v_local_vvc_cmd.msg) & format_command_idx(v_local_vvc_cmd), C_SCOPE, v_local_vvc_cmd.quietness);\n")
    print_linefeed(file_handle)
    file_handle.write("          when FLUSH_COMMAND_QUEUE =>\n")
    file_handle.write("            work.td_vvc_entity_support_pkg.interpreter_flush_command_queue(v_local_vvc_cmd, command_queue, vvc_config, vvc_status, C_VVC_LABELS);\n")
    print_linefeed(file_handle)
    file_handle.write("          when TERMINATE_CURRENT_COMMAND =>\n")
    file_handle.write("            work.td_vvc_entity_support_pkg.interpreter_terminate_current_command(v_local_vvc_cmd, vvc_config, C_VVC_LABELS, terminate_current_cmd, executor_is_busy);\n")
    print_linefeed(file_handle)
    file_handle.write("          when FETCH_RESULT =>\n")
    file_handle.write("            work.td_vvc_entity_support_pkg.interpreter_fetch_result(result_queue, v_local_vvc_cmd, vvc_config, C_VVC_LABELS, last_cmd_idx_executed, shared_vvc_response);\n")
    print_linefeed(file_handle)
...
def add_vvc_executor(file_handle, vvc_channel, features):
    ...
    file_handle.write("      -------------------------------------------------------------------------\n")
    file_handle.write("      work.td_vvc_entity_support_pkg.fetch_command_and_prepare_executor(v_cmd, command_queue, vvc_config, vvc_status, queue_is_increasing, executor_is_busy, C_VVC_LABELS);\n")
    print_linefeed(file_handle)
    ...
    file_handle.write("      --<USER_INPUT> Replace this if statement with a check of the current v_cmd.operation, in order to set v_cmd_is_bfm_access to true if this is a BFM access command\n")
    file_handle.write("      -- Example:\n")
    ...
    if number_of_executors > 1:
        ...
        file_handle.write("        --  -- If the result from the BFM call is to be stored, e.g. in a read call, use the additional procedure illustrated in this read example\n")
...
def add_vvc_pipeline_step(file_handle, queue_name, features):
    ...
    file_handle.write("      -------------------------------------------------------------------------\n")
    file_handle.write("      work.td_vvc_entity_support_pkg.fetch_command_and_prepare_executor(v_cmd, " + queue_name + "_queue, vvc_config, vvc_status, " + queue_name + "_queue_is_increasing, " + queue_name + "_is_busy, C_VVC_LABELS);\n")
    print_linefeed(file_handle)
    file_handle.write("      -- Select between a provided msg_id_panel via the vvc_cmd_record from a VVC with a higher hierarchy or the\n")
    file_handle.write("      -- msg_id_panel in this VVC's config. This is to correctly handle the logging when using Hierarchical-VVCs.\n")
    file_handle.write("      v_msg_id_panel := get_msg_id_panel(v_cmd, vvc_config);\n")
...
def add_leaf_vvc_entity(file_handle, vvc_name, channel):
    ...
    file_handle.write("    GC_" + vvc_name.upper() + "_CONFIG" + fill_with_n_spaces(vvc_name.__len__(), 24) + "=> GC_" + vvc_name.upper() + "_BFM_CONFIG,\n")
    ...
    file_handle.write("  -- Example:\n")
    file_handle.write("    -- " + vvc_name.lower() + "_vvc_" + channel.lower() + "         => " + vvc_name.lower() + "_vvc_if." + vvc_name.lower() + "_vvc_" + channel.lower() + ",\n")
...
def add_vvc_cmd_pkg_header(file_handle, features):
    ...
    file_handle.write("     generic map(t_generic_element => t_vvc_cmd_record,")
    file_handle.write("                 c_generic_default => C_VVC_CMD_DEFAULT);")
    file_handle.write("   use protected_vvc_cmd_pkg.all;")
    file_handle.write("   shared variable shared_vvc_cmd : protected_vvc_cmd_pkg.t_prot_generic_array;")
    print_linefeed(file_handle)
    file_handle.write("  " + division_line + "\n")
    ...
    file_handle.write("  use protected_vvc_response_pkg.all;\n")
    file_handle.write("  shared variable shared_vvc_response : protected_vvc_response_pkg.t_prot_generic_array;\n")
    print_linefeed(file_handle)
    file_handle.write("  " + division_line + "\n")
    ...
    file_handle.write("    generic map(t_generic_element => integer,\n")
    file_handle.write("                c_generic_default => -1);\n")
    file_handle.write("  use protected_vvc_last_received_cmd_idx_pkg.all;\n")
    print_linefeed(file_handle)
    file_handle.write("  shared variable shared_vvc_last_received_cmd_idx : protected_vvc_last_received_cmd_idx_pkg.t_prot_generic_array;\n")
    print_linefeed(file_handle)
    file_handle.write("  " + division_line + "\n")
...
def add_methods_pkg_header(file_handle, vvc_name, vvc_channels, features):
    ...
    print_linefeed(file_handle)
    file_handle.write("  signal " + vvc_name.upper() + "_VVCT" + fill_with_n_spaces(vvc_name.__len__(), 13) + ": t_vvc_target_record := set_vvc_target_defaults(C_VVC_NAME);\n")
    ...
    file_handle.write("    generic map(t_generic_element => t_vvc_status,\n")
    file_handle.write("                c_generic_default => C_VVC_STATUS_DEFAULT);\n")
    file_handle.write("  use protected_vvc_status_pkg.all;\n")
    file_handle.write("  shared variable shared_{}_vvc_status : protected_vvc_status_pkg.t_prot_generic_array;\n".format(vvc_name.lower()))
    file_handle.write("  alias shared_vvc_status is shared_{}_vvc_status; -- This alias is for internal use in the VVC\n".format(vvc_name.lower()))
    print_linefeed(file_handle)
    file_handle.write("  package protected_vvc_config_pkg is new uvvm_util.protected_generic_types_pkg\n")
    file_handle.write("    generic map(t_generic_element => t_vvc_config,\n")
    file_handle.write("                c_generic_default => C_{}_VVC_CONFIG_DEFAULT);\n".format(vvc_name.upper()))
    file_handle.write("  use protected_vvc_config_pkg.all;\n")
    file_handle.write("  shared variable shared_{}_vvc_config : protected_vvc_config_pkg.t_prot_generic_array;\n".format(vvc_name.lower()))
    file_handle.write("  alias shared_vvc_config is shared_{}_vvc_config; -- This alias is for internal use in the VVC\n".format(vvc_name.lower()))
    print_linefeed(file_handle)
    file_handle.write("  package protected_msg_id_panel_pkg is new uvvm_util.protected_generic_types_pkg\n")
    file_handle.write("    generic map(t_generic_element => t_msg_id_panel,\n")
    file_handle.write("                c_generic_default => C_VVC_MSG_ID_PANEL_DEFAULT);\n")
    file_handle.write("  use protected_msg_id_panel_pkg.all;\n")
    file_handle.write("  shared variable shared_{}_vvc_msg_id_panel : protected_msg_id_panel_pkg.t_prot_generic_array;\n".format(vvc_name.lower()))
    file_handle.write("  alias shared_vvc_msg_id_panel is shared_{}_vvc_msg_id_panel; -- This alias is for internal use in the VVC\n".format(vvc_name.lower()))
    print_linefeed(file_handle)
    if features["scoreboard"]:
        file_handle.write("  -- Scoreboard\n")
        file_handle.write("  package sbi_sb_pkg is new bitvis_vip_scoreboard.generic_sb_pkg\n")
        file_handle.write("    generic map(\n")
        ...
        file_handle.write("  shared variable " + vvc_name.upper() + "_VVC_SB  : " + vvc_name.lower() + "_sb_pkg.t_prot_generic_sb;\n")
        print_linefeed(file_handle)
    print_linefeed(file_handle)
    file_handle.write("  " + division_line + "\n")
...
def add_methods_pkg_body(file_handle, vvc_name, features):
    ...
    file_handle.write("  --   variable v_normalised_addr    : unsigned(C_VVC_CMD_ADDR_MAX_LENGTH-1 downto 0) := \n"
                      "  --            normalize_and_check(addr, shared_vvc_cmd.addr, ALLOW_WIDER_NARROWER, \"addr\", \"shared_vvc_cmd.addr\", proc_call & \" called with too wide addr. \" & msg);\n")
    file_handle.write("  --   variable v_normalised_data    : std_logic_vector(C_VVC_CMD_DATA_MAX_LENGTH-1 downto 0) := \n"
                      "  --            normalize_and_check(data, shared_vvc_cmd.data, ALLOW_WIDER_NARROWER, \"data\", \"shared_vvc_cmd.data\", proc_call & \" called with too wide data. \" & msg);\n")
    ...
    if features["transaction_info"]:
        ...
        file_handle.write("  begin\n")
        file_handle.write("  -- <USER_INPUT> Please insert the VVC operations here with the appropiate fields.\n")
        ...
        file_handle.write("  --      vvc_transaction_info_group.bt.address(vvc_cmd.addr'length-1 downto 0)    := vvc_cmd.addr;\n")
...
def add_transaction_pkg(file_handle, vvc_name, vvc_channels, features):
    ...
    if features["transaction_info"]:
        ...
        file_handle.write("  type t_transaction_status is (INACTIVE, IN_PROGRESS, FAILED, SUCCEEDED);\n")
        print_linefeed(file_handle)
        file_handle.write("  constant C_TRANSACTION_STATUS_DEFAULT : t_transaction_status := INACTIVE;\n")
        print_linefeed(file_handle)
        ...
        file_handle.write("    transaction_status  => C_TRANSACTION_STATUS_DEFAULT\n")
        file_handle.write("    );\n")
        print_linefeed(file_handle)
